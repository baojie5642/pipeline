...
public class C_Thread implements Runnable{
	...
	private C_Thread(final Object objectLock, final byte[] byteLock, final InnerLock innerLock) {
		this.objectLock0 = objectLock;
		this.byteLock1 = byteLock;
		this.innerLock2 = innerLock;
	}

	public static C_Thread build(final Object objectLock, final byte[] byteLock, final InnerLock innerLock) {
		C_Thread c_Thread = new C_Thread(objectLock, byteLock, innerLock);
		return c_Thread;
	}
	...
	public void run() {
		...
		while (true) {
			...
			System.out.println("lockOrder in cthread:"+lockOrder);
			if (lockOrder == 0) {
