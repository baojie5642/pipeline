...
public class B_Thread  implements Runnable{
	...
	private B_Thread(final Object objectLock, final byte[] byteLock, final InnerLock innerLock) {
		this.objectLock0 = objectLock;
		this.byteLock1 = byteLock;
		this.innerLock2 = innerLock;
	}

	public static B_Thread build(final Object objectLock, final byte[] byteLock, final InnerLock innerLock) {
		B_Thread b_Thread = new B_Thread(objectLock, byteLock, innerLock);
		return b_Thread;
	}
	...
	public void run() {
		...
		while (true) {
			...
			System.out.println("lockOrder in bthread:"+lockOrder);
			if (lockOrder == 0) {
