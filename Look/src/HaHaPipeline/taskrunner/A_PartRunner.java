...
public class A_PartRunner implements Runnable {

	private static final String partName = "APart";
	...
	public static A_PartRunner create(final OneLineInTask oneLineInTask) {
		return new A_PartRunner(oneLineInTask);
	}

	private A_PartRunner(final OneLineInTask oneLineInTask) {
	...
	private void work() {
		if (null == a_PartOfTask) {
			WriteLog.putErrorLog("volatile A_PartOfTask must not be null.");
		} else {
			statrWork();
			doWork();
			finishWork();
	...
	private void finishWork() {
		...
		if (true == isLastThread) {
			B_AutoMachine.putOneLine(oneLineInTask);// 如果本地线程是最后一个执行此部分的线程，那么由他来将line放入下一个部分
	...
	private boolean isIAmTheLastThreadDoThisPart() {
		final A_PartOfTask innerA_PartOfTask = a_PartOfTask;
		final boolean isLastThread = innerA_PartOfTask.isThisPartFinished();
		return isLastThread;
	}
